    ...
    >
      <div className="flex flex-col lg:flex-row gap-8">
            ...
            {days.map((day, index) => {
              ...
              return (
                <div 
                  ...
                  className={`border-r border-b border-slate-100 p-1 sm:p-2 transition-all cursor-pointer relative group ${
                    !day ? 'bg-slate-50/20' : isSelected ? 'bg-indigo-50/30' : 'hover:bg-slate-50/50'
                  }`}
                >
                  {day && (
                    <>
                      <div className="flex items-center justify-between mb-1">
                        <span className={`inline-flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 text-xs sm:text-sm font-bold rounded-lg ${
                          today ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : isSelected ? 'text-indigo-600' : 'text-slate-600'
                        }`}>
                          {day}
                        </span>
                        {dayTodos.length > 0 && (
                      ...
                      <div className="space-y-1 overflow-hidden">
                        ...
                        {dayTodos.length > 0 && dayAssignments.length < 2 && (
                          <div className="text-[8px] sm:text-[10px] text-slate-400 italic px-1">
        ...
        <div className="w-full lg:w-96 space-y-6">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[500px] lg:h-[600px]">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-lg font-bold text-slate-800">Tasks for</h3>
              <p className="text-sm text-indigo-600 font-semibold">{selectedDateStr}</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-hide">
              {selectedDay ? (
                    ...
                    <AnimatePresence mode="popLayout">
                      {selectedDayTodos.length > 0 ? (
                          ...
                          >
                            <button
                              ...
                              className={`flex-shrink-0 transition-colors ${
                                todo.completed ? 'text-emerald-500' : 'text-slate-300 hover:text-indigo-500'
                              }`}
                            >
                              {todo.completed ? (
                        ...
                        <div className="text-center py-12">
                          ...
                          <p className="text-xs text-slate-300 mt-1">Add one to get started!</p>
                        </div>
                      )}
                    </AnimatePresence>
                  </div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-slate-400">Select a date to manage tasks</p>
                </div>
              )}
            </div>
