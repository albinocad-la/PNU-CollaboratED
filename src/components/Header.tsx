    ...
    <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-10">
      <div className="flex items-center gap-2 sm:gap-4">
        {showBack ? (
          <button 
            onClick={onBack}
            className="p-2 text-slate-500 hover:bg-slate-100 rounded-xl transition-colors flex items-center gap-2 group"
          >
        ...
        <h1 className="text-xl sm:text-2xl font-semibold text-slate-800 tracking-tight truncate max-w-[150px] sm:max-w-none">{title}</h1>
      </div>
      ...
      <div className="flex items-center gap-3 sm:gap-6">
        <div className="relative hidden md:block">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="pl-10 pr-4 py-2 bg-slate-100 border-transparent rounded-full focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all w-48 lg:w-64 text-sm outline-none"
          />
        </div>
        ...
        <div className="relative" ref={notificationRef}>
          <button 
            ...
            className={`relative p-2 rounded-full transition-all ${isNotificationsOpen ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500 hover:bg-slate-100'}`}
          >
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
              <span className="absolute top-1.5 right-1.5 flex h-4 w-4 items-center justify-center bg-red-500 rounded-full border-2 border-white text-[10px] font-bold text-white">
                {unreadCount}
              </span>
            )}
          </button>
                  ...
                  {pendingAssignments.length > 0 ? (
                    <div className="divide-y divide-slate-50">
                      {pendingAssignments.map((assignment) => {
                        ...
                        return (
                          <div key={assignment.id} className="p-4 hover:bg-slate-50 transition-colors cursor-pointer group">
                            <div className="flex gap-3">
                              <div className={`mt-1 p-2 rounded-xl shrink-0 ${
                                isOverdue ? 'bg-red-50 text-red-500' : 
                                isSoon ? 'bg-amber-50 text-amber-500' : 
                    ...
                    <div className="p-8 text-center">
                      ...
                      <p className="text-xs text-slate-500 mt-1">No pending assignments found.</p>
                    </div>
                  )}
                </div>
        ...
        <button 
          ...
          className="flex items-center gap-3 sm:pl-4 sm:border-l border-slate-200 cursor-pointer hover:opacity-80 transition-opacity text-left"
        >
          <img
            src={photoURL}
            alt="User avatar"
            className="w-8 h-8 sm:w-9 h-9 rounded-full object-cover border border-slate-200"
